...
Run this **code** in console after website is loaded, Now **check all images links** present on the website at **bottom**.

```js
var ilist = document.images;
for (var i = 0; i < ilist.length; i++) {
  console.log(ilist[i].src);
  console.log(ilist[i].alt);
  const isrc = document.createTextNode(ilist[i].src);
